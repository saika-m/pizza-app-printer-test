...
def check_internet_connection(url="https://www.google.com", timeout=3):
    """
    Check if there is an internet connection by performing a HEAD request to Google.
    """
    try:
...
def format_receipt_text(order):
    ...
    if isinstance(items, list):
        for item in items:
            ...
            size = item.get('size', '14') # Default to 14 if missing
            price = item.get('price', 0)
            
            receipt_lines.append(f"{qty}x {name} ({size}\")"[:22].ljust(22) + f" ${price * qty:.2f}")
            ...
            if customizations:
                ...
                if customizations.get('toppings'):
                    for topping in customizations['toppings']:
                        receipt_lines.append(f"   + {topping}")
